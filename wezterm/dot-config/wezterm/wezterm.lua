...
local config = {
	...
	keys = {
		...
		{ key = "s", mods = "CTRL|ALT", action = act.ShowLauncherArgs({ flags = "WORKSPACES" }) },
		{
			...
			action = act.PromptInputLine({
				description = wezterm.format({
					...
					{ Text = "Enter name for (new) workspace to switch to" },
				}),
		...
		{ key = "a", mods = "CTRL|ALT", action = act.SwitchToWorkspace({ name = "default" }) },
		{
			...
			action = act.PromptInputLine({
				description = wezterm.format({
					{ Attribute = { Intensity = "Bold" } },
					{ Foreground = { AnsiColor = "Fuchsia" } },
					{ Text = "Rename the current workspace" },
				}),
				action = wezterm.action_callback(function(window, pane, line)
					-- line will be `nil` if they hit escape without entering anything
					if line then
		...
		{
			-- spawn tab and set its name immediately
			key = "t",
			mods = "CTRL|ALT",
			action = act.Multiple({
...
-- cause the gui to match the rename behavior of ctrl+alt+t
